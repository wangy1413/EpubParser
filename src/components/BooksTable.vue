...
const batchExportCoverImages = async () => {
  ...
  // 数量限制检查
  if (props.books.length > 100) {
    alert('单次导出封面图数量不能超过100本，请选择更少的书籍')
    return
  }

  try {
    coverExportLoading.value = true
    coverExportProgress.value = 0
    currentExportIndex.value = 0

    const coverImages = []

    // 逐个解析并收集封面图
    for (const [index, book] of props.books.entries()) {
      currentExportIndex.value = index + 1
      coverExportProgress.value = Math.round(((index + 1) / props.books.length) * 100)

      try {
        // 解析EPUB文件，提取封面图
        const parseResult = await parseEpubFile(book.path)

        if (parseResult.coverImage) {
          // 收集封面图数据
          const blob = base64ToBlob(parseResult.coverImage)
          if (blob) {
            const extension = getImageExtension(parseResult.coverImage)
    ...
    coverExportLoading.value = false
    coverExportProgress.value = 0
    currentExportIndex.value = 0
  }
}
...
// 下载单个封面图
const downloadCoverImage = (coverImage, title) => {
  try {
    // 创建下载链接
    ...
    // 使用书名作为文件名，处理特殊字符
    const fileName = (title || '未知书名').replace(/[\\/:*?"<>|]/g, '') + '.' + getImageExtension(coverImage)
...
const exportToExcel = async () => {
  ...
  try {
    ...
    for (const [index, book] of props.books.entries()) {
      try {
        // 解析EPUB文件，不提取封面图以提高性能
        const parseResult = await parseEpubFile(book.path, false)
